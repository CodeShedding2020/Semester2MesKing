...
namespace Messenger_Kings.Controllers
{
    ...
    public class BookingsController : Controller
    {
        ...
        public void AddPoints(string uid, double price)
        {
            Points points = (from c in db.Points
                             where c.Client_ID == uid
                             select c).SingleOrDefault();
            var getClient = (from g in db.Clients
                             where g.Client_ID == uid
                             select g.ClientCategory.ClientCat_Type).SingleOrDefault();

            if (getClient.ToUpper() == "Pay as you go".ToUpper())
            {
                points.ClientPoints += (int)(price * .03);
            }
            else
            {
                points.ClientPoints += (int)(price * .05);
            }
            db.SaveChanges();
        }
